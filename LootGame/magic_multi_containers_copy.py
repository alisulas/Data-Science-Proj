# Load items and all container (single, multi, magic, magic multi). choose container
# Loot item: Place item in first compartment with capacity (no weight increase if magic).
# List looted items: Show container and compartment details (no weight increase in magic).
...
class Container:
    ...
    def __str__(self, level=0):
        indent = "    " * level
        items_str = "\n".join([f"{indent}    {item}" for item in self.items]) if self.items else "No items"
...
# Define the MagicMultiContainer class inheriting from Container
class MagicMultiContainer(Container):
    def __init__(self, name, empty_weight=0):
        super().__init__(name, empty_weight, capacity=0)  # Capacity is managed by sub-containers

    def add_item(self, item):
        """Attempt to add the item to the first sub-container with enough capacity."""
...
class MagicContainer(Container):
    ...
    def add_item(self, item):
        """Attempt to add the item to the sub-container with enough capacity."""
        for sub_container in self.sub_containers:
            if sub_container.add_item(item):
                return True
        return False
...
# Load compartments data from containers.csv
...
# Load single-compartment containers from containers.csv
...
# Load multi-compartment containers from multi_containers.csv
...
# Load magic multi-compartment containers from magic_multi_containers.csv
...
# Load magic containers from magic_containers.csv
...
# Load magic containers from magic_containers.csv
def load_magic_containers_v2(filename, compartments_data, container_map):
    """
    Alternative loader if 'magic_containers.csv' has multiple sub-containers per container.
    Modify this function based on the exact structure of your 'magic_containers.csv'.
    """
    # Implementation can be added here if 'magic_containers.csv' requires handling multiple sub-containers.
...
def select_container(all_containers):
    container_names = [container.name for container in all_containers.values()]
    while True:
...
def main():
    ...
    load_multi_containers('data/multi_containers.csv', compartments_data, container_map)
